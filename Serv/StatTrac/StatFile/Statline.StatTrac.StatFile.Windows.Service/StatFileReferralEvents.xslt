...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="ReferralEvents">
    ...
    <xsl:copy-of select="$columnDelimeter"/>
    <xsl:copy-of select="ds:ReferralID"/>
    <xsl:copy-of select="$columnDelimeter"/>
  ...
  <xsl:template name="ReferralEvents2004">
    <xsl:param name="eventDetailDataTable" />
    <!-- Calls the previous file version-->
    <xsl:call-template name="ReferralEvents" >
      <xsl:with-param name="eventDetailDataTable" select="exsl:node-set($eventDetailDataTable)"/>
    </xsl:call-template>
    <xsl:copy-of select="$columnDelimeter"/>
    <!-- call column delimeter between each template-->
    <xsl:copy-of select="ds:ReferralEventAttnTo"/>
    <xsl:copy-of select="$columnDelimeter"/>
    <xsl:copy-of select="ds:ReferralEventCalloutMin"/>
    <xsl:copy-of select="$columnDelimeter"/>
    <xsl:copy-of select="ds:ReferralEventCalloutAfter"/>
    <xsl:copy-of select="$columnDelimeter"/>
    <xsl:copy-of select="ds:ReferralEventContactConfirm"/>
    <xsl:copy-of select="$columnDelimeter"/>
    <xsl:copy-of select="ds:ReferralEventFaxNbr"/>
    <xsl:copy-of select="$columnDelimeter"/>
    <xsl:copy-of select="ds:ReferralEventDocName"/>
    <!-- do not include a columnDelimeter on end of column definition templateds-->
  </xsl:template>
</xsl:stylesheet>
