...
IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id(N'[dbo].[sps_FSCallCountSummary_A3]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[sps_FSCallCountSummary_A3];
GO

CREATE PROCEDURE sps_FSCallCountSummary_A3
...
BEGIN
	...
	DROP TABLE IF EXISTS #Referral_FSCallCountSummary_Filtered;
	...
	SELECT rfccs.*		
	INTO #Referral_FSCallCountSummary_Filtered
	FROM 
    ...
    FROM 		
		#Referral_FSCallCountSummary_Filtered AS Referral_FSCallCountSummary
		JOIN		vwDataWarehouseWebReportGroupOrg ON vwDataWarehouseWebReportGroupOrg.OrganizationID = Referral_FSCallCountSummary.OrganizationID
		JOIN		vwDataWarehouseSourceCode sc ON sc.SourceCodeID = Referral_FSCallCountSummary.SourceCodeID
