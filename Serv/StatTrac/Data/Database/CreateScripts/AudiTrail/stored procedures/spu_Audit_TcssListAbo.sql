 /****************************************************************************** 
**  File:  
**  Name: spu_Audit_TcssListAbo.sql 
**  Desc:  
** 
**  Date:    Author:    Description: 
**  --------   --------   ------------------------------------------- 
**  06/27/2011  jim hawkins   Update Audit records for DBO.TcssListAbo**   
*******************************************************************************/
SET QUOTED_IDENTIFIER ON   
 GO
   SET ANSI_NULLS ON     
 GO
   if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spu_Audit_TcssListAbo]') and OBJECTPROPERTY(id,N'IsProcedure') = 1) 
 BEGIN
 drop procedure [dbo].[spu_Audit_TcssListAbo]
 PRINT 'Drop Sproc: spu_Audit_TcssListAbo'  
 END   
 GO
 PRINT 'Create Sproc: spu_Audit_TcssListAbo'  
 GO
   create procedure "spu_Audit_TcssListAbo"  
   (   @TcssListAboId int  
