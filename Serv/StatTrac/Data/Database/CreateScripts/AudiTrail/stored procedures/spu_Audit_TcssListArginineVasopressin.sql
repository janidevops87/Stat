 /****************************************************************************** 
**  File:  
**  Name: spu_Audit_TcssListArginineVasopressin.sql 
**  Desc:  
** 
**  Date:    Author:    Description: 
**  --------   --------   ------------------------------------------- 
**  06/27/2011  jim hawkins   Update Audit records for DBO.TcssListArginineVasopressin**   
*******************************************************************************/
SET QUOTED_IDENTIFIER ON   
 GO
   SET ANSI_NULLS ON     
 GO
   if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spu_Audit_TcssListArginineVasopressin]') and OBJECTPROPERTY(id,N'IsProcedure') = 1) 
 BEGIN
 drop procedure [dbo].[spu_Audit_TcssListArginineVasopressin]
 PRINT 'Drop Sproc: spu_Audit_TcssListArginineVasopressin'  
 END   
 GO
 PRINT 'Create Sproc: spu_Audit_TcssListArginineVasopressin'  
 GO
   create procedure "spu_Audit_TcssListArginineVasopressin"  
...
CASE WHEN SUBSTRING(@Bitmap, 1, 1) & 64 = 64 AND ISNULL(@UnosValue, '') = '' THEN 'ILB'  ELSE @UnosValue END  )  END    
 GO
