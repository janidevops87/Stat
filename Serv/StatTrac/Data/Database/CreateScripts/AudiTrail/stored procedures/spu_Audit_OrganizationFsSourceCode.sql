...
**  
**  File:   
**  Name: spu_Audit_OrganizationFsSourceCode.sql  
**  Desc:   
**  
**  Date:     	Author:    		Description:  
**  --------    --------   		-------------------------------------------  
**  06/15/2011  Steve Barron  	Update Audit records for DBO.OrganizationFsSourceCode
**    	
*******************************************************************************/
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spu_Audit_OrganizationFsSourceCode]') and OBJECTPROPERTY(id,N'IsProcedure') = 1) 
	BEGIN 
		drop procedure [dbo].[spu_Audit_OrganizationFsSourceCode]
		PRINT 'Drop Sproc: spu_Audit_OrganizationFsSourceCode'  
	END   
GO

PRINT 'Create Sproc: spu_Audit_OrganizationFsSourceCode'  
GO

create procedure "spu_Audit_OrganizationFsSourceCode"  
(   
...
AND SUBSTRING(@Bitmap, 1, 1) & 8 <> 8 --FsSourceCodeId  
...
+ ISNULL(CAST(@SourceCodeId AS VARCHAR(1)), '')
+ ISNULL(CAST(@FsSourceCodeId AS VARCHAR(1)), '')
...
CASE WHEN SUBSTRING(@Bitmap, 1, 1) & 4 = 4 AND ISNULL(@SourceCodeId, 0) = 0 THEN -2 ELSE @SourceCodeId END, 
CASE WHEN SUBSTRING(@Bitmap, 1, 1) & 8 = 8 AND ISNULL(@FsSourceCodeId, 0) = 0 THEN -2 ELSE @FsSourceCodeId END, 
