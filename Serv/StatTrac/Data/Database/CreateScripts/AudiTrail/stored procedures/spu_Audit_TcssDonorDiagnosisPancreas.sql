 /****************************************************************************** 
**  File:  
**  Name: spu_Audit_TcssDonorDiagnosisPancreas.sql 
**  Desc:  
** 
**  Date:    Author:    Description: 
**  --------   --------   ------------------------------------------- 
**  06/22/2011  jim hawkins   Update Audit records for DBO.TcssDonorDiagnosisPancreas**   
*******************************************************************************/
SET QUOTED_IDENTIFIER ON   
 go
   SET ANSI_NULLS ON     
 go
   if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spu_Audit_TcssDonorDiagnosisPancreas]') and OBJECTPROPERTY(id,N'IsProcedure') = 1) 
 begin
 drop procedure [dbo].[spu_Audit_TcssDonorDiagnosisPancreas]
 PRINT 'Drop Sproc: spu_Audit_TcssDonorDiagnosisPancreas'  
 END   
 go
 PRINT 'Create Sproc: spu_Audit_TcssDonorDiagnosisPancreas'  
 go
   create procedure "spu_Audit_TcssDonorDiagnosisPancreas"  (   @TcssDonorDiagnosisPancreasId int  
...
+ ISNULL(CAST(@Comment AS VARCHAR(1)), '')
   insert into DBO.TcssDonorDiagnosisPancreas  (   "TcssDonorDiagnosisPancreasId" 
