 /****************************************************************************** 
**  File:  
**  Name: spu_Audit_TcssDonorVitalSignDetail.sql 
**  Desc:  
** 
**  Date:    Author:    Description: 
**  --------   --------   ------------------------------------------- 
**  06/23/2011  jim hawkins   Update Audit records for DBO.TcssDonorVitalSignDetail**   
*******************************************************************************/
SET QUOTED_IDENTIFIER ON   
 GO
   SET ANSI_NULLS ON     
 GO
   if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spu_Audit_TcssDonorVitalSignDetail]') and OBJECTPROPERTY(id,N'IsProcedure') = 1) 
 BEGIN
 drop procedure [dbo].[spu_Audit_TcssDonorVitalSignDetail]
 PRINT 'Drop Sproc: spu_Audit_TcssDonorVitalSignDetail'  
 END   
 GO
 PRINT 'Create Sproc: spu_Audit_TcssDonorVitalSignDetail'  
 GO
   create procedure "spu_Audit_TcssDonorVitalSignDetail"  
...
AND SUBSTRING(@Bitmap, 1, 1) & 16 <> 16 --TcssDonorVitalSignId  
AND SUBSTRING(@Bitmap, 1, 1) & 32 <> 32 --TcssListVitalSignId  
...
+ ISNULL(CAST(@TcssDonorVitalSignId AS VARCHAR(1)), '') 
+ ISNULL(CAST(@TcssListVitalSignId AS VARCHAR(1)), '') 
...
CASE WHEN SUBSTRING(@Bitmap, 1, 1) & 16 = 16 AND ISNULL(@TcssDonorVitalSignId, 0) = 0 THEN -2 ELSE @TcssDonorVitalSignId END, 
CASE WHEN SUBSTRING(@Bitmap, 1, 1) & 32 = 32 AND ISNULL(@TcssListVitalSignId, 0) = 0 THEN -2 ELSE @TcssListVitalSignId END, 
