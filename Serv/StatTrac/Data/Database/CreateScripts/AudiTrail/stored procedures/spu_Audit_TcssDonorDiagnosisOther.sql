 /****************************************************************************** 
**  File:  
**  Name: spu_Audit_TcssDonorDiagnosisOther.sql 
**  Desc:  
** 
**  Date:    Author:    Description: 
**  --------   --------   ------------------------------------------- 
**  06/22/2011  jim hawkins   Update Audit records for DBO.TcssDonorDiagnosisOther**   
*******************************************************************************/
SET QUOTED_IDENTIFIER ON   
 go
   SET ANSI_NULLS ON     
 go
   if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spu_Audit_TcssDonorDiagnosisOther]') and OBJECTPROPERTY(id,N'IsProcedure') = 1) 
 begin
 drop procedure [dbo].[spu_Audit_TcssDonorDiagnosisOther]
 PRINT 'Drop Sproc: spu_Audit_TcssDonorDiagnosisOther'  
 END   
 go
 PRINT 'Create Sproc: spu_Audit_TcssDonorDiagnosisOther'  
 go
   create procedure "spu_Audit_TcssDonorDiagnosisOther"  (   @TcssDonorDiagnosisOtherId int  
...
AND SUBSTRING(@Bitmap, 1, 1) & 16 <> 16 --TcssListOtherOrganId  
...
+ ISNULL(CAST(@TcssListOtherOrganId AS VARCHAR(1)), '') 
...
CASE WHEN SUBSTRING(@Bitmap, 1, 1) & 16 = 16 AND ISNULL(@TcssListOtherOrganId, 0) = 0 THEN -2 ELSE @TcssListOtherOrganId END, 
