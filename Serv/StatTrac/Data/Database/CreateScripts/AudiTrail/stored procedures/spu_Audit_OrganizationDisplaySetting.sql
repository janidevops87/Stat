...
**  
**  File:   
**  Name: spu_Audit_OrganizationDisplaySetting.sql  
**  Desc:   
**  
**  Date:     	Author:    		Description:  
**  --------    --------   		-------------------------------------------  
**  06/15/2011  Steve Barron  	Update Audit records for DBO.OrganizationDisplaySetting
**    
*******************************************************************************/
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spu_Audit_OrganizationDisplaySetting]') and OBJECTPROPERTY(id,N'IsProcedure') = 1) 
	BEGIN 
		drop procedure [dbo].[spu_Audit_OrganizationDisplaySetting]
		PRINT 'Drop Sproc: spu_Audit_OrganizationDisplaySetting'  
	END   
GO

PRINT 'Create Sproc: spu_Audit_OrganizationDisplaySetting'  
GO

create procedure "spu_Audit_OrganizationDisplaySetting"  
(   
...
AND SUBSTRING(@Bitmap, 1, 1) & 4 <> 4 --DashBoardDisplaySettingId  
...
+ ISNULL(CAST(@DashBoardDisplaySettingId AS VARCHAR(1)), '')
...
CASE WHEN SUBSTRING(@Bitmap, 1, 1) & 4 = 4 AND ISNULL(@DashBoardDisplaySettingId, 0) = 0 THEN -2 ELSE @DashBoardDisplaySettingId END, 
