...
**  
**  File:   
**  Name: spu_Audit_AlertSourceCode.sql  
**  Desc:   
**  
**  Date:     	Author:    		Description:  
**  --------    --------   		-------------------------------------------  
**  06/17/2011  Steve Barron  	Update Audit records for DBO.AlertSourceCode
**    
*******************************************************************************/
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spu_Audit_AlertSourceCode]') and OBJECTPROPERTY(id,N'IsProcedure') = 1) 
	BEGIN 
		drop procedure [dbo].[spu_Audit_AlertSourceCode]
		PRINT 'Drop Sproc: spu_Audit_AlertSourceCode'  
	END   
GO

PRINT 'Create Sproc: spu_Audit_AlertSourceCode'  
GO

create procedure "spu_Audit_AlertSourceCode"  
(   
...
IF NOT 
(
SUBSTRING(@Bitmap, 1, 1) & 2 <> 2 --AlertID  
