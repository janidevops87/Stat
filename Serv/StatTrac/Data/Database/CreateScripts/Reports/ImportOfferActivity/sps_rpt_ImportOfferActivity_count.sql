IF EXISTS (SELECT *
           FROM   DBO.SYSOBJECTS
           WHERE  ID = OBJECT_ID(N'[dbo].[sps_rpt_ImportOfferActivity_count]')
...
CREATE TABLE #sps_rpt_ImportOfferActivityCountResults
   (
	...
	--				INSERT #sps_rpt_ImportOfferActivityCountResults 
	--					EXEC _ReferralProdArchive..sps_rpt_ImportOfferActivity_count_select
	...
	--				INSERT #sps_rpt_ImportOfferActivityCountResults 
	--					EXEC sps_rpt_ImportOfferActivity_count_select
	--						@UserOrgID, @ReportGroupID, @CallID, @StartDateTime, @EndDateTime, @SourceCodeName, @MessageCallerOrganization, @MessageFor,@MessageForOrganizationID, @DisplayMT
	--			END
...
SELECT ISNULL(SUM(RecordCount), 0) AS 'RecordCount' FROM #sps_rpt_ImportOfferActivityCountResults

DROP TABLE #sps_rpt_ImportOfferActivityCountResults      
