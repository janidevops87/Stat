...
**	09/28/2015		jegerberich	Removed length restriction from CallTypeName logic. VS 23002
*******************************************************************************/ 
...
--set @CallStartDateTime = '1/1/2014'
--set @CallEndDateTime = '1/1/2015'
--set @ReportGroupID = NULL
--set @TrackingTypeID = 15 --StatTrac
...
SELECT DISTINCT 
	...
	CASE
		WHEN ISNUMERIC(QATrackingNumber) = 1
		THEN
			(
				SELECT TOP 1 ISNULL(MessageType.MessageTypeName, ReferralType.ReferralTypeName)
				FROM
					ReferralType
					INNER JOIN Referral ON ReferralType.ReferralTypeID = Referral.ReferralTypeID
					INNER JOIN WebReportGroupOrg ON Referral.ReferralCallerOrganizationID = WebReportGroupOrg.OrganizationID
					RIGHT OUTER JOIN MessageType
							INNER JOIN [Message] ON MessageType.MessageTypeID = [Message].MessageTypeID 
								RIGHT OUTER JOIN CallType
									INNER JOIN [Call]
										INNER JOIN SourceCode ON [Call].SourceCodeID = SourceCode.SourceCodeID
									ON CallType.CallTypeID = [Call].CallTypeID
								ON [Message].CallID = [Call].CallID 
						ON Referral.CallID = [Call].CallID
				WHERE
					[call].callid  = QATrackingNumber
			)
		ELSE 'N/A'
	END AS CallTypeName,
