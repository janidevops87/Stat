if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[sps_rpt_MessageDetail_Count]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
	BEGIN
		drop procedure [dbo].[sps_rpt_MessageDetail_Count]
		PRINT 'Dropping Procedure: sps_rpt_MessageDetail_Count'
	END
...
CREATE PROCEDURE sps_rpt_MessageDetail_Count
	@UserOrgID		int = Null,
...
CREATE TABLE #sps_rpt_MessageDetailCountResults
   (
	...
	--				INSERT #sps_rpt_MessageDetailCountResults 
	--					EXEC _ReferralProdArchive..sps_rpt_MessageDetail_count_select
	--						@UserOrgID, @ReportGroupID, @CallID, @StartDateTime, @EndDateTime, @SourceCodeName, @DisplayMT


	--				/* run in production database */
	--				INSERT #sps_rpt_MessageDetailCountResults 
	--					EXEC sps_rpt_MessageDetail_count_select
	--						@UserOrgID, @ReportGroupID, @CallID, @StartDateTime, @EndDateTime, @SourceCodeName, @DisplayMT
	--			END
...
SELECT SUM(RecordCount) AS 'RecordCount' FROM #sps_rpt_MessageDetailCountResults

DROP TABLE #sps_rpt_MessageDetailCountResults      
