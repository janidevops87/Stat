 if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[GetUserInformationById]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
drop procedure [dbo].[GetUserInformationById]
GO
...
CREATE PROCEDURE GetUserInformationById
...
AND
	wp.WebPersonLastSessionAccess BETWEEN DATEADD( hh, -1, getdate()) AND getdate()
AND 
