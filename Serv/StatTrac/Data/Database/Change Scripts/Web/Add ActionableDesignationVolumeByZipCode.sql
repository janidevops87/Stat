...
SELECT
    @ReportDisplayName          = 'Actionable Designation Volume by Zip Code',
    @ReportLocalOnly            = 0,
    @ReportVirtualURL           = '/StatTrac/ActionableDesignationVolumeByZipCode',
    @ReportTypeID               = 6,
    @Unused                     = null,
...
--- customParamsRegistryState 
SET @ReportControlName  = 'customParamsRegistryZipCode'
IF NOT EXISTS (SELECT * FROM ReportControl WHERE ReportControlName like @ReportControlName)
BEGIN
	insert ReportControl (ReportControlName, ReportParamSectionID) 
	values (@ReportControlName, 1) 
