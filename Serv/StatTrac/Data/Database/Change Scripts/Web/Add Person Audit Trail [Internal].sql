...
SELECT
    @ReportDisplayName          = 'Person Audit Trail [Internal]',
    @ReportLocalOnly            = 0,
    @ReportVirtualURL           = '/StatTrac/PersonAuditTrail',
    @ReportTypeID               = 5,
    @Unused                     = null,
...
IF NOT EXISTS (SELECT * FROM ReportControl WHERE ReportControlName like 'customParamsPerson')
BEGIN
	insert ReportControl (ReportControlName, ReportParamSectionID) 
	values ('customParamsPerson', 1) 
END

select @ReportControlID = ReportControlID from ReportControl WHERE ReportControlName = 'customParamsPerson'
if not exists( select * from reportparamconfiguration where reportID = @ReportID and reportcontrolID = @ReportControlID)
BEGIN
	exec spi_ReportParamConfiguration
		@ReportID			,
...
select		
	@sortTypeName = 'LastName',
	@SorTypeDisplayName = 'Last Name'
...
END
select		
	@sortTypeName = 'FirstName',
	@SorTypeDisplayName = 'First Name'
