...
SELECT
    @ReportDisplayName          = 'Family Services Summary',
    @ReportLocalOnly            = 0,
    @ReportVirtualURL           = '/StatTrac/FS_Summary',
    @ReportTypeID               = 4,
    @Unused                     = null,
...
if not exists(select * from report where replace(ReportDisplayName, ' (new)', '') = @ReportDisplayName and (ReportVirtualURL = @ReportVirtualURL OR ReportVirtualURL = REPLACE(@ReportVirtualURL, '/StatTrac/', '/StatTrac Test/')) )
BEGIN
	exec SPI_Report 
                  @ReportID OUTPUT            ,
                  @ReportDisplayName          ,
...
INSERT @controlList
VALUES('customParamsFSSummary', 1)
