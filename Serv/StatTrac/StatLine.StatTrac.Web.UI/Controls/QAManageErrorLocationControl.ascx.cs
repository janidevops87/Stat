...
{
    public partial class QAManageErrorLocation : BaseUserControlSecure
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!this.IsPostBack)
            {
                ...
                QAUpdateManager.FillQAErrorLocation(dsQAUpdateData, OrgID, Convert.ToInt16(cbxDisplayAll.Checked),0);
                gridErrorLocations.DataBind();
            }
        ...
        protected void Button1_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect("~/QAManageErrorLocation.aspx?OrgID=" + orgID + "&OrgName=" + ddlOrganization.DisplayValue);
        }
        
        protected void gridErrorLocations_UpdateRowBatch(object sender, RowEventArgs e)
        {
            int orgID = 0;
            if (ddlOrganization.SelectedRow != null)
            {
            ...
            if (String.IsNullOrEmpty(gridErrorLocations.Rows[e.Row.Index].Cells[1].Text))
            {
                DisplayMessages.Add(MessageType.ErrorMessage, "You must enter a description");
                return;
            }
            QAUpdateData.QAErrorLocationRow row;
            row = dsQAUpdateData.QAErrorLocation.NewQAErrorLocationRow();
            ...
            else
            {
                ...
                row = dsQAUpdateData.QAErrorLocation.FindByQAErrorLocationID(
                    (int)e.Row.Cells[gridErrorLocations.Columns.IndexOf("QAErrorLocationID")].Value); 
                row["LastStatEmployeeID"] = Page.Cookies.StatEmployeeID;
            ...
            if (e.Row.DataChanged == DataChanged.Added)
            {
                row["QAErrorLocationActive"] = 1;
                // if the row is a new row, add the row to the dataset
                dsQAUpdateData.QAErrorLocation.AddQAErrorLocationRow(row);
            }
            
            try
            {
                QAUpdateDB.UpdateQAErrorLocation(dsQAUpdateData);
        ...
        {
            this.dsQAUpdateData = new Statline.StatTrac.Data.Types.QAUpdateData();
            QAUpdateManager.FillQAErrorLocation(dsQAUpdateData, Convert.ToInt32(ddlOrganization.SelectedRow.Cells[1].Value), Convert.ToInt16(cbxDisplayAll.Checked),0);
            gridErrorLocations.DataBind();
        }

        protected void gridErrorLocations_InitializeLayout1(object sender, LayoutEventArgs e)
        {
            this.gridErrorLocations.UpdateRowBatch += new Infragistics.WebUI.UltraWebGrid.UpdateRowBatchEventHandler(this.gridErrorLocations_UpdateRowBatch);
            this.gridErrorLocations.Columns.Band.BaseTableName = "QAErrorLocation";
            this.gridErrorLocations.Rows.Band.BaseTableName = "QAErrorLocation";
            this.gridErrorLocations.Rows.Band.Key = "QAErrorLocation";
        }
