...
Function GetDetailList()
	...
	END IF	
     ...
     ' build sourcecode list
	   ...
	   'ccarroll 07/05/06 added check for new or existing NOK data
	...
	'drh FSMOD 05/21/03 - Temporarily commented out HeartBeat column and added placeholder; change to go live with HeartBeat
	'lvQueryString = lvQueryString & "ReferralDOB, '' 'ReferralDonorHeartBeat', "  
	'bjk 05/01/03 additional fields T.T uncomment heartbeat 03/19/2004
	...
	'ccarroll, 03/21/06 - added new fields for StatTrac 8.0 per Requirements document
	...
	lvQueryString = lvQueryString & "Referral.ReferralDonorNameMI "
	   ...
	   lvQueryString = lvQueryString & "FROM Call WITH (nolock) JOIN Referral ON Referral.CallID = Call.CallID JOIN Secondary ON Referral.CallID = Secondary.CallID "
	   ...
	   lvQueryString = lvQueryString & "JOIN CallCriteria CC (nolock) ON CC.CallID = Call.CallID "
	   lvQueryString = lvQueryString & "JOIN ServiceLevel (nolock) ON ServiceLevel.ServiceLevelID = CC.ServiceLevelID "
	   lvQueryString = lvQueryString & "JOIN ServiceLevelSourceCode (nolock) ON ServiceLevelSourceCode.ServiceLevelID = ServiceLevel.ServiceLevelID "
	   lvQueryString = lvQueryString & "AND ServiceLevelSourceCode.SourceCodeID = Call.SourceCodeID "
	   lvQueryString = lvQueryString & "JOIN ServiceLevelCustomField (nolock) ON ServiceLevelCustomField.ServiceLevelID = ServiceLevelSourceCode.ServiceLevelID "
