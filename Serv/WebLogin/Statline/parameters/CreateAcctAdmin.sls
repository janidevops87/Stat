...
Dim rsDeleteSource
Dim rsSourceUsers
Dim rsSourceCodeEditMode
Dim rsCheckExists
Dim rsGroupOwnerFill
Dim vEditUserID
Dim rsSourceUserSEdit
Dim GroupOwnerSwitchBlank
Dim DefaultSourceCode
 

	'*********************************************************************************
	'Name: Createacct.asp 
	'Date Created: 4/30/2005                          Created By: T.T 11/30/2005
	'Release #: [Release OnlineHospital Created For  ex: 1.1]    Task: [Task created for]
...
'Begin Page Code
			...
			if Request.Form("Defaultsource") > 0 then
					DefaultSourceCode = Request.Form("Defaultsource") 
			end if
			 
			GroupOwnerSwitch = False
			GroupOwnerSwitchBlank = False
			'Set GroupOwner Variables
			if Request.Form("GroupOwner") <> Request.Form("GroupOwnerLast") then
					GroupOwnerSwitch = True	
					GroupOwnerSwitchBlank = True
					Print GroupOwnerSwitchBlank
			end if
				...
				if Request.Form("GroupOwner") > 0 then
						GroupOwnerValue =  Request.Form("GroupOwner")
				else
						GroupOwnerValue = 0
				end if
			
				if Request.Form("ReportGroup") > 0 then
						ReportGroupValue = Request.Form("ReportGroup")
				else
						ReportGroupValue = 0
				end if
    ...
    if EditBooln = true then
    		Set cmd = Server.CreateObject("ADODB.Command")
				...
				if rsEditUser.EOF = False then
							...
							vEditUserID = rsEditUser("WebPersonID")	
    ...
    if Request.Form("AddButton") = "       Add      " then 
						...
						'Print "Pressed Add"
							...
							'Response.Write("The add " & Request.Form("AddSource")(a) & " was selected<BR>")
									...
									cmd.CommandText = "Select * from onlinehospitalsourcecodewebperson where webpersonID = " & Request.Form("EditUserID") & " and sourceCodeID = " & Request.Form("AddSource")(a) 
									print cmd.CommandText
									Set rsCheck = cmd.Execute

									if rsCheck.EOF = True then
										cmd.CommandText = "insert into onlinehospitalsourcecodewebperson (webpersonID,sourceCodeID,SourceCodeName) values ("& Request.Form("EditUserID") &"," & Request.Form("AddSource")(a) & ",'Blank')"
										'print cmd.CommandText
		...
		if Request.Form("RemoveButton") = " Remove " then
						...
						For a = 1 to Request.Form("RemoveSource").Count
							'Response.Write("The Remove " & Request.Form("RemoveSource")(a) & " was selected<BR>")
									...
									Set cmd = Server.CreateObject("ADODB.Command")
									cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
									cmd.CommandText = "delete from OnlineHospitalSourceCodeWebPerson where webpersonID = " & Request.Form("EditUserID") & " and sourcecodeID = " & Request.Form("RemoveSource")
									Set rs = cmd.Execute
					Next
						else
		...
		'Edit Person button available
		'*******************************
		if EditBooln = True  then
			     		Print "This is in the EditUserMode"
			     		Print	Request.Form("EditUserID")
			     		Set cmd = Server.CreateObject("ADODB.Command")
			 								cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
			 								cmd.CommandText = "select distinct ohswp.SourceCodeID,SourceCode.SourceCodeName from onlinehospitalsourcecodewebperson ohswp join SourceCode ON SourceCode.SourceCodeID = ohswp.SourceCodeID where webpersonID = " & Request.QueryString("EditUser") & " order by SourceCode.SourceCodeName"
			 								 
			 				SET rsSourceUserSEdit = cmd.Execute
		    end if
		    ...
		    if Request.Form("vButtonCaption") = "Edit" then
		    		...
		    		Set cmd = Server.CreateObject("ADODB.Command")
										cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
										cmd.CommandText = "select Distinct ohswp.SourceCodeID,SourceCode.SourceCodeName from onlinehospitalsourcecodewebperson ohswp join SourceCode ON SourceCode.SourceCodeID = ohswp.SourceCodeID where webpersonID = " & Request.Form("EditUserID") & " order by SourceCode.SourceCodeName"
										vEditUserID = Request.Form("EditUserID")
						SET rsSourceUserSEdit = cmd.Execute
    end if
    '******************************
...
If Len(Request.Form("SRCHUSER")) > 0 Then
	...
	cmd.Parameters("@userId").Value = Request.QueryString("UserID")
...
If Len(Request.QueryString("EditUser")) > 0 Then
	...
	If Not rsC.EOF Then  
	   ...
	   vStatlinePerson = rsC("OrganizationId")
	...
	'Get Default Source Code for Edit Mode
	Set cmd = Server.CreateObject("ADODB.Command")
	cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
							'
	cmd.CommandText = "select SourceCodeID from OnlineHospitalSourceCodeWebPerson where WebPersonID = '" & WebPersonUserID & "'" 
	set rsSourceCodeEditMode = cmd.Execute
	Do While Not rsSourceCodeEditMode.EOF 
						If Not rsSourceCodeEditMode.EOF Then
						'Print rsSourceCodeEditMode("SourceCodeID")
							
						end if
						rsSourceCodeEditMode.MoveNext
	  Loop
End If
'*******************************'*******************************
...
If UCase(Request.Form("submitform")) = "CREATE ACCOUNT" Or UCase(Request.Form("submitform")) = "EDIT ACCOUNT" Then
	'SELECT StatEmployee.StatEmployeeUserId FROM StatEmployee WHERE StatEmployee.StatEmployeeUserId 
	...
	if vPersonOrgId = 0 then
	 if Request.Form("vStatlinePerson") >0 then
	 		vPersonOrgID = Request.Form("vStatlinePerson")
	 end if
	end if
	...
	cmd.Parameters("@DefaultSourcecodeID").Value = DefaultSourceCode
	...
	cmd.CommandText = "delete from onlinehospitalsourcecodewebperson where webpersonID = " & rsWebPerson("WebPersonID") & " and Locked <> 1 "
	...
	SET rsSourceOwnerRem = cmd.Execute
		...
		Do While Not rsSourceOwnerRem.EOF 
					If Not rsSourceOwnerRem.EOF Then
						...
						cmd.CommandText = "insert into onlinehospitalsourcecodewebperson (webpersonID,sourceCodeID,SourceCodeName,Locked) values (" & rsWebPerson("WebPersonID") & "," & rsSourceOwnerRem("SourcecodeID") & ",'" & rsSourceOwnerRem("SourceCodeName") & "',1)"
...
<%    End If %>
	    ...
	    <%'Show the source code boxes because this is a statline user
	    If Request.Querystring("UserOrgID") = 194 Then%>
	    	                  ...
	    	                  <td><div class="body_background">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;*&nbsp;Source&nbsp;Code:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div></td>
														 ...
														 <td>												
																	...
																	<%if Request.Form("vButtonCaption") <> "Edit" then
																		...
																		<%if Request.Form("vButtonCaption") = "Edit" then
																						'rsSourceUser.MoveFirst
																						Do While Not rsSourceUserSEdit.EOF 
																									If Not rsSourceUserSEdit.EOF Then
																										Print rsSourceUserSEdit("SourceCodeName")
																									End If   %>
																									<OPTION VALUE=<%=rsSourceUserSEdit("SourceCodeID")%>><%=rsSourceUserSEdit("SourceCodeName")%></OPTION>

																									<%rsSourceUserSEdit.MoveNext
																						Loop  
																		End if%>
	                    ...
	                    <td>
	                       <table border="0"> 
	                          <tr><input type="hidden" name="PERSONID" value="<%=vPersonId%>">
	                          		<input type="hidden" name="EditUserID" value="<%=vEditUserID%>">
	                          		<input type="hidden" name="vStatlinePerson" value="<%=vStatlinePerson%>">
	                             ...
	                             <SELECT NAME="GroupOwner" ONCHANGE="document.forms[1].submit();">
															 		...
															 		<%
															 			'Fill Group Owner DropDown (Scenerio StatTracUser)
															 			'*******************************
															 			if EditBooln <> True then
																		Set cmd = Server.CreateObject("ADODB.Command")
																		cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
																		...
																		<%End if%>
																					...
																					<%'Edit user Fill SourceCode twins
																							if EditBooln = True then
																									Set cmd = Server.CreateObject("ADODB.Command")
																									cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
																									cmd.CommandText = "select ohswp.SourceCodeID,SourceCode.SourceCodeName from onlinehospitalsourcecodewebperson ohswp join SourceCode ON SourceCode.SourceCodeID = ohswp.SourceCodeID where webpersonID = " & Request.QueryString("EditUser") & " order by SourceCode.SourceCodeName"						 
																									SET rsSourceOwner = cmd.Execute
																									counter = 0
																				...
																				<%'Edit user Fill SourceCode twins
																					if Request.Form("vButtonCaption") = "Edit" then
																							Set cmd = Server.CreateObject("ADODB.Command")
																							cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
																							cmd.CommandText = "select ohswp.SourceCodeID,SourceCode.SourceCodeName from onlinehospitalsourcecodewebperson ohswp join SourceCode ON SourceCode.SourceCodeID = ohswp.SourceCodeID where webpersonID = " & Request.Form("EditUserID") & " order by SourceCode.SourceCodeName"						 
																							SET rsSourceOwner = cmd.Execute
																							counter = 0
																					...
																					<%'Prefill GroupOwner (PreFill scenerio)
																					if Request.QueryString("UserOrgID") <> 194 then
																						Set cmd = Server.CreateObject("ADODB.Command")
																						cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
																						cmd.CommandText = "select OnlineHospitalAccount.GroupOwnerID as GroupOwner, Organization.OrganizationName as OrgName from OnlineHospitalAccount join Organization ON OnlineHospitalAccount.GroupOwnerID = Organization.OrganizationID where webpersonID = " & Request.Querystring("UserID")
																						SET rsGroupOwnerFill = cmd.Execute
																						  
																							If Not rsGroupOwnerFill.EOF Then%>
																									<OPTION VALUE=<%=rsGroupOwnerFill("GroupOwner")%><% Response.Write(" selected")%>><%=rsGroupOwnerFill("OrgName")%></OPTION> 
																						
																							<%end if
																					end if
																				
																					
																					'If User Edit (Edit scenerio)
																						'*******************************
																							if EditBooln = True then
																				...
																				'If GroupOwner Value,(Refresh scenerio) Page is refreshed This is when Page is refreshed from adding or removing Sourcecodes
																				'*******************************
	                          ...
	                          <tr>  
	                             ...
	                             <td>
	                             <select name="ReportGroup"  ONCHANGE="document.forms[1].submit()">
	                             		<option value=0></option>
	                             		<%if GroupOwnerSwitchBlank = True then%>
	                             											 
																	<%end if%>
	                             		
	                             		<%if GroupOwnerValue =0 then 
	                             					'Prefill ReportGroup(Prefill scenerio)
	                             					'************************************************************************
																				
																				if Request.QueryString("UserOrgID") <> 194 then
																						if rsGroupOwnerFill("GroupOwner")	> 0 then%>
																						    <option value=0></option>	
																								<%Set cmd = Server.CreateObject("ADODB.Command")
																								cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
																								cmd.CommandText = "SELECT DISTINCT WebReportGroupID, WebReportGroupName FROM WebReportGroup WHERE OrgHierarchyParentID = " & rsGroupOwnerFill("GroupOwner") & " ORDER BY WebReportGroupName" 
	                             			...
	                             			<%else%>
	                             					...
	                             					'Fill ReportGroup DropDown(StatTrac Scenerio:Main)
	                             					'*******************************
	                             					<% GroupOwnerSwitch = False%>
	                             					...
	                             					<%Set cmd = Server.CreateObject("ADODB.Command")
																	...
																	<%end if
																				...
																				%>
																	<%'If user is being edited(Edit scenerio)
																	'*******************************
																	if EditBooln = True then%>
															...
															'*******************************
																					%>
															<%'If ReportGroup value has value in it, maintain state(Refresh scenerio)
															'*******************************
															if ReportGroupValue > 0 then		
																		Set cmd = Server.CreateObject("ADODB.Command")
																		cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
																		cmd.CommandText = "select WebReportGroupID,WebReportGroupName from webreportgroup where webreportgroupID =" & Request.Form("ReportGroup") 
																		set rsReportGroupSelect = cmd.Execute

																		if Request.Form("RemoveButton") = " Remove " then
																				'nothing
																		else 
																				if GroupOwnerSwitchBlank = False then
																					If Not rsReportGroupSelect.EOF Then%>
																							<option value=<%=Request.Form("ReportGroup")%><% Response.Write(" selected")%>><%=rsReportGroupSelect("WebReportGroupName")%></option>
																					<%End If
																				End If
																		end if
																	        ...
																	        <select name="GroupOwnerHospital">
																									...
																									<%else%>
																												...
																												<% 
																													...
																													'(StatTrac scenerio:Main) User has just piced a ReportGroup
																													'**********************************************************
																														if ReportGroupValue > 0 then
																																Set cmd = Server.CreateObject("ADODB.Command")
																																cmd.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
																									...
																									<%'Edit if user is being edited(Edit Scenerio)
																									'*******************************
																									if EditBooln = True then
	                              ...
	                              <td><div class="body_background">*&nbsp;Default&nbsp;SourceCode:</div></td>
	                                      ...
	                                      <td>
																			 <SELECT NAME="Defaultsource" >
																				...
																				<%'Fill in Deault source code dropdown(StatTrac scenerio:Main) Get sourcecode after reportGroup is chosen
																			...
																			%>
																							...
																							<%'Edit default source code drop down(Edit Scenerio)
																								'*******************************
																								if EditBooln = True then
...
function validateCreation()
{
	...
	var _hospitalOwner = document.acctform.GroupOwnerHospital.options[document.acctform.GroupOwnerHospital.selectedIndex].value

  var _GroupOwners = document.acctform.GroupOwner.options[document.acctform.GroupOwner.selectedIndex].value

	var _ReportOwners = document.acctform.ReportGroup.options[document.acctform.ReportGroup.selectedIndex].value
					...
					else
					{
						...
						else
						{
							...
							else
							{
								...
								if(_GroupOwners == 0)
												{
												alert("GroupOwner must be chosen");
										...
										if(_ReportOwners == 0)
													{
													alert("ReportGroup must be chosen");
