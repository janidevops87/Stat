...
''If IsNumeric
'UserID = FormatNumber(Request("UserID"),0,,0,0)
'Response.Write("<br>UserId: " & Cstr(UserId))
%>
...
If AuthorizeMain Then
   ...
   'Verify Access
	 									...
	 									Set RSTimeRepeat = Server.CreateObject("ADODB.Recordset")
															 ...
															 if rstimerepeat.eof = false then
          	...
          	'Recordset Get organization list
     			...
     			If Orgchoice <> "" and PersonChoice<>"" then
					 Set RSIns = Server.CreateObject("ADODB.Recordset")
 																		...
 																		cmd2.Parameters("@OrganizationID").Value = orgchoice
																		'request("OrganizationName_" & orgchoice)
				...
				Set RSEmails = Server.CreateObject("ADODB.Recordset")
																...
																cmd2.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD
																vQ = "SELECT Distinct Person.PersonFirst, WebPerson.WebpersonEmail, Person.PersonID, CASE WHEN Person.Inactive = 1 THEN Person.PersonFirst+' '+PersonLast + ' (Inactive)' ELSE Person.PersonFirst+' '+PersonLast END  AS Person, PersonType.PersonTypeName FROM Person JOIN Organization ON Organization.OrganizationID = Person.OrganizationID Join WebPerson ON person.personid = webperson.personid JOIN PersonType ON PersonType.PersonTypeID = Person.PersonTypeID WHERE Person.OrganizationID = " & userOrgID &" AND Person.Inactive = 0 ORDER BY PersonFirst"
																'''Response.Write "xxxxxxxx<br><br>" & vq & "<BR><BR>"
																cmd2.CommandText = vQ
																Set RSEmails = cmd2.execute  
...
%>
<form action="LifeBreach.sls?<%'UserID==UserID%>xxx=xxx&UserOrgID=<%=UserOrgID%>" name="frmLifeBreach" method="post">
<%
...
<TD><input type="Hidden" name="OrganizationIDDis_<%=RSREc("OrganizationID")%>" value="<%=RSREc("OrganizationID")%>" size="10" maxlength="20" readonly/></TD>
...
<option value="">------</option>
<%' Added hard-coded mail group.  5/6/05 - SAP %>
<option value="Mail Group">Mail Group</option> 
			 	<%Do While Not RSEmails.EOF%>             
            <option value="<%=Replace(RSEmails("Person"),"*","")%>"><%=Replace(RSEmails("Person"),"*","")%></option> 
            <%RSEmails.MoveNext%>
        <%
        Loop	 
        ...
        RSEmails.MoveFirst
        Do while Not RSEmails.EOF
        	' Kluge:  Their emails are often longer than the 30 chars provided in the webpersonemail field, so add ".org" when needed.  5/5/05 - SAP
        	If LCase(Right(RSEmails("webpersonemail"), 19)) = "@giftoflifemichigan" Then
        		sThisEmail = RSEmails("webpersonemail") & ".org"
        	ElseIf LCase(Right(RSEmails("webpersonemail"), 15)) = "@giftoflifemich" Then
        		sThisEmail = RSEmails("webpersonemail") & "igan.org"        		
        	Else
			sThisEmail = RSEmails("webpersonemail")        	
        	End If  %>
        		<input type="hidden" name="PersonID_<%=Replace(RSEmails("Person"),"*","")%>" value="<%=sThisEmail%>" size="255" maxlength="255" />
						<%RSEmails.MoveNext%>
<%
	Loop
%>
<%' Added hard-coded mail group email address.  5/6/05 - SAP %>
<input type="hidden" name="PersonID_Mail Group" value="Non-DR@GiftOfLifeMichigan.Org"/>
</TD>
<TR>
<TD></TD>
</TR>
