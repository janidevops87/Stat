...
If AuthorizeMain Then
  ...
  ' Now that updates/inserts/deletes are complete get the record set
    ...
    ' Pick the FunctionIDs 1=Navigation, 2=Reports, 3=Buttons, 4=Functions, 5=Online Referral
    ...
    IF request("permtype") = "4" then
       ...
       RSFUNCTION.Open()
    End IF
    ' Online Referral PermissionType added 4/5/05 - SAP
    IF request("permtype") = "5" Then  
       Set RSFUNCTION = Server.CreateObject("ADODB.Recordset")
       RSFUNCTION.ActiveConnection = "DSN=" & TRANSACTIONDSN & ";UID=" & DBUSER & ";Password=" & DBPWD 
       RSFUNCTION.Source = "{call dbo.SPS_OnlineReferral}"
       RSFUNCTION.CursorType = 0
       RSFUNCTION.CursorLocation = 2
       RSFUNCTION.LockType = 1
       RSFUNCTION.Open()
...
<%END IF
' Online Referral PermissionType added 4/5/2005 - SAP
IF request("permtype") = "5" then%>
   <option value="<%=RSFUNCTION("OnlinePermissionID")%>" <%IF RSPERM("FunctionId") = RSFUNCTION("OnlinePermissionID") then response.write("selected") end if%>><%=RSFUNCTION("OnlinePermissionName")%></option>
<%END IF
RSFUNCTION.MOVENEXT
Loop
...
<%END IF
IF request("permtype") = "5" then%>
   <option value="<%=RSFUNCTION("OnlinePermissionID")%>"><%=RSFUNCTION("OnlinePermissionName")%></option>
<%END IF%>
<%
RSFUNCTION.MOVENEXT
Loop
