	...
	If VerifyReferralTypeAccess(True) Then
		...
		If CountCheck = "False" Then
			...
			%>
				...
				<!-- Print the header. -->
				<!--#include virtual="/loginstatline/reports/referral/Head.sls"-->

				<!-- Get the counts of each referral type. -->
				<%If pvShowSection1 Then%>
					<!--#include virtual="/loginstatline/reports/referral/ActivitySection1_1.sls"-->
				<%
				End If
				...
				%>
					...
					<!--#include virtual="/loginstatline/reports/referral/ActivitySection2_1.sls"-->
				<%
				End If
...
Function ExecuteMain()
	...
	vQuery = "sps_ReferralActivity " & pvReportGroupID & ", '" & pvStartDate &"', " & "'" & pvEndDate &"', " &  pvOrgID & ", " & vTZ &", " & pvReferralTypeID &", " & "'" & pvOrderBy &"', " & pvUserOrgID
