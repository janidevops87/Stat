...
If AuthorizeMain Then
    ...
    'Check the Event Log View Permission
    Set RSEventLog = Server.CreateObject("ADODB.Recordset")
    ...
    cmd.Parameters("@FUNCTIONID").Value = 1 'FunctionID of the Event Log View
    Set RSEventLog = cmd.Execute
    IF Not RSEventLog.EOF then
    	 
        EventLogTest=RSEventLog("FunctionString")
    ...
    If VerifyReferralTypeAccess(True) Then
        ...
        If CountCheck = "False" Then
            ...
            If ExecuteMain Then
                ' Test to see if this is a GOLM Org
                vQuery = "select * from sourcecodeorganization "
                vQuery = vQuery & "WHERE organizationid=" & pvUserOrgID & " " 
                vQuery = vQuery & "AND organizationid IN ( "    
                vQuery = vQuery & "select so.organizationid "
                vQuery = vQuery & "from sourcecodeorganization so "
                vQuery = vQuery & "join organization o on so.organizationid = o.organizationid "
                vQuery = vQuery & "where so.sourcecodeid IN (251) "
                vQuery = vQuery & ") "
            ...
            %>
                <!-- Print the header. -->
                <!--#include virtual="/loginstatline/reports/referral/DetailHead_1.sls"-->
                ...
                <%Else
                    ...
                    'bjk 11/7/05: adding flush so Referral Detail can run without timing out or overfilling the buffer
                    response.flush
