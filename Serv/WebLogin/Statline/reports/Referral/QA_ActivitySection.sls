...
Do Until RS.EOF = True
	...
	<tr>
	    ...
	    <td ALIGN=LEFT VALIGN=TOP width="10%"><FONT FACE="<%=FontNameData%>" SIZE="<%=FontSizeData%>">
 		<a target="ReferralDetail" href="/loginstatline/reports/referral/detail_1.sls?CallID=<%=RS("CallID")%>&amp;UserID=<%=Request.QueryString("UserID")%>&amp;UserOrgID=<%=Request.QueryString("UserOrgID")%>&amp;DSN=<%=DataSourceName%>&amp;Options=0&NoLog=1&CountCheck=False"><%=RS("CallNumber")%></a></FONT></td>
	    <td ALIGN=LEFT VALIGN=TOP width="7%"><FONT FACE="<%=FontNameData%>" SIZE="<%=FontSizeData%>"><%=RS("ReferralTypeName")%></FONT></td>
