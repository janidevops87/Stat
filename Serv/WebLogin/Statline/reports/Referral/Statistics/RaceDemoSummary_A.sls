...
<%	
	...
	'drh 1/1/04 - Added If statement to prevent error when there are no records
	If Not RS.EOF Then
		Call TableHeader()
	End If
...
%>
				...
				<TR>
					...
					<TD <%=vCurrentColor%> ALIGN=Left><FONT SIZE="1" FACE="<%=FontNameHead%>">
								...
								<A target="ReferralSummary" HREF="../summary_A.sls?<%=("Type=2&StartDate=" & Request.QueryString("StartDate") & "&EndDate=" & Request.QueryString("EndDate") & "&ReportGroupID=" & pvReportGroupID & "&OrgID=" & Request.QueryString("OrgID") & "&userID=" & Request.QueryString("UserID") & "&userOrgID=" & Request.QueryString("UserOrgID") & "&DSN=" & Request.QueryString("DSN") & "&Options=&ReportID=11&NoLog=1&AND=_AND_ReferralDonorGender_=_" & "'" & RS("DonorGender") & "'" &"_AND_ReferralDonorRaceID_=_" & RS("RaceID") & "_")%>
								<%=vAnd%>"> 
								<%=RS("AllTypes")%>
								</A></FONT>
					</TD>
