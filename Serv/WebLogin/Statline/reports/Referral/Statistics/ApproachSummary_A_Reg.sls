...
If AuthorizeMain Then
	...
	'Set Title Values	
	vMainTitle = "Approach Summary (All Types - w/Reg)"
	...
	pvTempEndDate = Request.Querystring("EndDate")
	
	'Trim off the time
	pvTempStartDate = mid(pvTempStartDate,1, len(pvTempStartDate)-6)
	pvTempEndDate = mid(pvTempEndDate,1, len(pvTempEndDate)-6)
...
%>
	...
	<TR><TD ALIGN=Left width="30%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>&nbsp</B></TD>
		...
		<TD ALIGN=Left width="9%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>Registered</B></TD>
	...
	<TR><TD COLSPAN=11><HR ALIGN=CENTER COLOR="#000000" NOSHADE SIZE=1 WIDTH="100%"></TD></TR>
...
<%
	...
	Do While Not RS.EOF
		...
		if pvTempStartDate < pvEndDate Then
			' Only recalculate the end date if more than one month is involved 
			' and the report is for a single OrgId.  9/3/04 - SAP
			If Request.Querystring("OrgID") > 0 And DateDiff("m", pvTempStartDate, pvTempEndDate) > 1 Then
				pvTempEndDate = Month(pvTempStartDate) & "/" & CalculateDaysInMonth(pvTempStartDate) & "/" & Year(pvTempStartDate)
			End If
		
			pvNewQueryString = pvNewQueryString & "StartDate=" & pvTempStartDate & "_00:00" & "&"
			pvNewQueryString = pvNewQueryString & "EndDate=" & pvTempEndDate & "_23:59" & "&"
			...
			'Char Chaput 01/18/2005 HD Case 1731 Drill down didn't work because the start and end date 
			'where getting stripped to summary_a.sls due to querystring rebuild and date calcs 
			'so added tothis else statement.
			pvNewQueryString = pvNewQueryString & "StartDate=" & pvTempStartDate & "_00:00" & "&"
			pvNewQueryString = pvNewQueryString & "EndDate=" & pvTempEndDate & "_23:59" & "&"
		End If		
...
%>
			...
			<%
			'Registry
			vTotals(6) = vTotals(6) + RS("RegistryApproach")
			%>			
			<TD <%=vCurrentColor%>>
	...
	<TR><TD><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>Totals</B></TD>
		...
		<TD ALIGN=Left><FONT SIZE="1" FACE="<%=FontNameHead%>"><B><%=vTotals(6)%></B></FONT></TD>
