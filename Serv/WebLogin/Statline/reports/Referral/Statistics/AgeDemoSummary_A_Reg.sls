...
If AuthorizeMain Then
	...
	'Set Title Values	
	vMainTitle = "Age Demographics - w/Reg"
...
<%
	...
	Do While Not RS.EOF
		IF RS("DonorGender")= "M" Then
			...
			If IsNull(RS("AllTypesAge_Reg")) Then 
				vAverage(0,0) = 0
			Else
				vAverage(0,0)= RS("AllTypesAge_Reg")
			End IF
			If IsNull(RS("AppropriateOrganAge_Reg")) Then
				vAverage(1,0)= 0
			Else
				vAverage(1,0)= RS("AppropriateOrganAge_Reg")
			End IF
			If IsNull(RS("AppropriateAllTissueAge_Reg")) Then
				vAverage(2,0)= 0
			Else
				vAverage(2,0)= RS("AppropriateAllTissueAge_Reg")
			End IF	
			If IsNull(RS("AppropriateEyesAge_Reg")) Then
				vAverage(3,0)= 0
			Else
				vAverage(3,0)= RS("AppropriateEyesAge_Reg")		
			End IF	
			If IsNull(RS("AppropriateROAge_Reg")) Then
				vAverage(4,0)= 0
			Else
				vAverage(4,0)= RS("AppropriateROAge_Reg")
			End IF	
		ELSE
			If IsNull(RS("AllTypesAge_Reg")) Then 
				vAverage(0,1) = 0
			Else
				vAverage(0,1)= RS("AllTypesAge_Reg")
			End IF
			If IsNull(RS("AppropriateOrganAge_Reg")) Then
				vAverage(1,1)= 0
			Else
				vAverage(1,1)= RS("AppropriateOrganAge_Reg")
			End IF
			If IsNull(RS("AppropriateAllTissueAge_Reg")) Then
				vAverage(2,1)= 0
			Else
				vAverage(2,1)= RS("AppropriateAllTissueAge_Reg")
			End IF	
			If IsNull(RS("AppropriateEyesAge_Reg")) Then
				vAverage(3,1)= 0
			Else
				vAverage(3,1)= RS("AppropriateEyesAge_Reg")		
			End IF	
			If IsNull(RS("AppropriateROAge_Reg")) Then
				vAverage(4,1)= 0
			Else
				vAverage(4,1)= RS("AppropriateROAge_Reg")
			End IF	
		END IF
