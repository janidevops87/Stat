...
<title>Call Person Summary A</title>
</head>
<body bgcolor="#F5EBDD">
...
If AuthorizeMain Then
	...
	'Set Title Values	
	vMainTitle = "Caller Person Summary"
...
%>
	...
	<TR>
		<TD ALIGN=Left width="32%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>&nbsp &nbsp Caller Person</B></TD>
...
<%
	...
	vQuery = "sps_CallerPersonSummary_A " & pvReportGroupID & ", '" & pvStartDate & "', '" & pvEndDate & "', " & pvOrgID
...
%>
		...
		%>	
					...
					<TD <%=vCurrentColor%> ALIGN=Left><FONT SIZE="1" FACE="<%=FontNameHead%>">
						<% If pvOrgID <> 0 Then %>
							<A target="ReferralSummary" HREF="../summary_A.sls?<%=("Type=2&StartDate=" & RS("MonthID")& "/1/" & RS("YearID")& "_" &  "00:00"  & "&EndDate=" & RS("MonthID")+1& "/1/" & RS("YearID")& "_" &  "00:00"  & "&ReportGroupID=" & pvReportGroupID & "&OrgID=" & RS("OrganizationID") & "&userID=" & Request.QueryString("UserID") & "&userOrgID=" & Request.QueryString("UserOrgID") & "&DSN=" & Request.QueryString("DSN") & "&Options=&ReportID=11&NoLog=1&AND=_AND_ReferralCallerOrganizationID_=_"& RS("OrganizationID") &"__AND_ReferralCallerPersonID_=_" & RS("CallerPersonID") & "_")%>
							<%=vAnd%>"> 
							<%=RS("AllTypes")%>
						<% Else %>
		...
		If pvOrgID <> 0 AND DatePart("m",pvStartDate) <> DatePart("m",pvEndDate) Then
			For i = 0 To Ubound(CallPersonArray, 2)
				If CallPersonArray(0,i) = RS("PersonName") Then
					...
					CallPersonArray(7,i)= RS("CallerPersonID")
					Exit For
		...
		%>
				<TR>
					<TD <%=vCurrentColor%> ALIGN=Left><FONT SIZE="1" FACE="<%=FontNameHead%>">&nbsp &nbsp &nbsp &nbsp 
						<A target="ReferralSummary" HREF="../summary_A.sls?<%=("Type=2&StartDate=" & Request.QueryString("StartDate") & "&EndDate=" & Request.QueryString("EndDate") & "&ReportGroupID=" & pvReportGroupID & "&OrgID=" & CallPersonArray(6,i) & "&userID=" & Request.QueryString("UserID") & "&userOrgID=" & Request.QueryString("UserOrgID") & "&DSN=" & Request.QueryString("DSN") & "&Options=&ReportID=11&NoLog=1&AND=_AND_ReferralCallerOrganizationID_=_"& CallPersonArray(6,i) &"__AND_ReferralCallerPersonID_=_" & CallPersonArray(7,i) & "_")%>
						<%=vAnd%>"> 
...
Private Function AutoSort(SortThis)
	Dim Loop0
	Dim Loop1
	Dim Loop2
	Dim ArrayHold
	...
	For Loop0 = LBound(SortThis,2) to Ubound(SortThis,2)
	
		For Loop1 = Loop0 + 1 to Ubound(SortThis,2)
			IF STRCOMP(SortThis(0, loop0),SortThis(0, loop1),1) = 1 Then
				
				ReDim ArrayHold(Ubound(SortThis, 1))
				For Loop2 = 0 to Ubound(SortThis,1)
					ArrayHold(Loop2) = SortThis(Loop2,Loop0)
				Next
				For Loop2 = 0 to Ubound(SortThis,1)
					SortThis(Loop2,Loop0) = SortThis(Loop2,Loop1)
				Next				
				For Loop2 = 0 to Ubound(SortThis,1)
					SortThis(Loop2,Loop1) = ArrayHold(Loop2) 
				Next			
			End If
