...
<title>Family Services Referral Summary</title>
</head>
<body bgcolor="#F5EBDD">
...
%>
	...
	<TR align="top">
		<%If pvOrgID <> 0 Then%>
			<TD ALIGN=Left valign="top" width="32%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>Year / Month</B></TD>
		<%Else%>
			<TD ALIGN=Left valign="top" width="32%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>Referral Facility</B></TD>
		<%End If%>
		
		<TD ALIGN=Left valign="top" width="9%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>Total Family<br>Service</B></TD>
		<TD ALIGN=Left COLSPAN=2 valign="top" width="15%"><FONT SIZE="1" FACE="<%=FontNameHead%>">Secondary<br>Billable</TD>
		<TD ALIGN=Left COLSPAN=2 valign="top" width="15%"><FONT SIZE="1" FACE="<%=FontNameHead%>">Family<br>Approached</TD>
		<TD ALIGN=Left COLSPAN=2 valign="top" width="15%"><FONT SIZE="1" FACE="<%=FontNameHead%>">Medical/Social<br>History</TD>
	</TR>
	<TR><TD COLSPAN=9><HR ALIGN=CENTER COLOR="#000000" NOSHADE SIZE=1 WIDTH="100%"></TD>
	</TR>
...
<%
	...
	'to be processed
	vQuery = "sps_ReferralFSSummary_A " & pvReportGroupID & ", '" & pvStartDate & "', '" & pvEndDate & "', " & pvOrgID
	'Print vQuery	
...
%>
		<TR>
			<TD <%=vCurrentColor%>><FONT SIZE="1" FACE="<%=FontNameHead%>">
				<%If pvOrgID <> 0 Then Response.Write(RS("MonthYear")) Else	Response.Write(RS("OrganizationName"))%>	
			</TD>
			<TD <%=vCurrentColor%> ALIGN=Left><FONT SIZE="1" FACE="<%=FontNameHead%>"><I><%=RS("FST")%></I></FONT></TD>
          <%vTotals(0) = vTotals(0) + RS("FST")%>
	<!-- Approach Types -->
			<TD <%=vCurrentColor%>><FONT SIZE="1" FACE="<%=FontNameHead%>"><%=RS("fsSecondary")%></FONT></TD>
			<%If RS("FST") = 0 Then%>
				<TD <%=vCurrentColor%>><I><FONT SIZE="1" FACE="<%=FontNameHead%>">0%</FONT></I></TD>	
			<%Else%>
				<TD <%=vCurrentColor%>><I><FONT SIZE="1" FACE="<%=FontNameHead%>"><%=FormatNumber((RS("fsSecondary")/RS("FST"))*100,0)%>%</FONT></I></TD>
			<%End If%>
			<%vTotals(1) = vTotals(1) + RS("fsSecondary")%>
			
			<TD <%=vCurrentColor%>><FONT SIZE="1" FACE="<%=FontNameHead%>"><%=RS("fsApproach")%></FONT></TD>
			<%If RS("FST") = 0 Then%>
				<TD <%=vCurrentColor%>><I><FONT SIZE="1" FACE="<%=FontNameHead%>">0%</FONT></I></TD>			
			<%Else%>			
				<TD <%=vCurrentColor%>><I><FONT SIZE="1" FACE="<%=FontNameHead%>"><%=FormatNumber((RS("fsApproach")/RS("FST"))*100,0)%>%</FONT></I></TD>
			<%End If%>
			<%vTotals(2) = vTotals(2) + RS("fsApproach")%>
			
			<TD <%=vCurrentColor%>><FONT SIZE="1" FACE="<%=FontNameHead%>"><%=RS("fsBillMedSoc")%></FONT></TD>
			<%If RS("FST") = 0 Then%>
				<TD <%=vCurrentColor%>><I><FONT SIZE="1" FACE="<%=FontNameHead%>">0%</FONT></I></TD>			
			<%Else%>			
				<TD <%=vCurrentColor%>><I><FONT SIZE="1" FACE="<%=FontNameHead%>"><%=FormatNumber((RS("fsBillMedSoc")/RS("FST"))*100,0)%>%</FONT></I></TD>
			<%End If%>
			<%vTotals(3) = vTotals(3) + RS("fsBillMedSoc")%>
				
     <A></FONT></TD>
     <TD <%=vCurrentColor%>>&nbsp</TD>
