	...
	If VerifyReferralTypeAccess(True) Then
		...
		If CountCheck = "False" Then
			...
			%>
				...
				<!-- Print the header. -->
				<!--#include virtual="/loginstatline/reports/referral/ActivityHead.sls"-->
						
				<!-- Get the counts of each referral type. -->
				<%If pvShowSection1 Then%>
					<!--#include virtual="/loginstatline/reports/referral/ActivitySection1.sls"-->
				<%
				End If
				...
				%>
					...
					<!--#include virtual="/loginstatline/reports/referral/ActivitySection2.sls"-->
				<%
				End If
...
Function ExecuteMain()
	...
	ElseIf pvOrgID <> 0  Then
		...
		If RS.EOF = True Then
			...
			vErrorMsg = vErrorMsg & "Please contact Statline at (888) 881-7828 for assistance.</B> <BR> <BR> <BR>"
			vErrorMsg = vErrorMsg & "<FONT SIZE=1 FACE=Arial FONT COLOR = Black>"
			vErrorMsg = vErrorMsg & "Error:     (100, GetOrganizationName, Activity.sls) <BR> <BR>"
			vErrorMsg = vErrorMsg & "</FONT></FONT>"
			Response.Write(vErrorMsg)
			ExecuteMain = False
			Exit Function
	...
	ErrorReturn = Referral.GetActivity(ResultArray, vTZ, ,pvStartDate, pvEndDate, pvReportGroupID, pvOrgID, pvReferralTypeID, ,pvOrderBy, pvUserOrgID,DataSourceName)
