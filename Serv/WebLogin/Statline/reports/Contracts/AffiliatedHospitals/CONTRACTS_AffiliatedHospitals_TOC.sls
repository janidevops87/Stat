...
Set RS = Conn.Execute("CONTRACTS_AffiliatedHospitals_TOC")
...
<a CLASS="SideBarData" TARGET=page href="CONTRACTS_AffiliatedHospitals_ByClient.sls?UserID=<%=UserID%>&UserOrgID=<%=UserOrgID%>&AsgChoice=*&ReportID=<%=ReportID%>&ReportName=<%=ReportName%>">All Clients</a><br>
<%Do While Not RS.EOF%>
	<a CLASS="SideBarData" TARGET=page href="CONTRACTS_AffiliatedHospitals_ByClient.sls?UserID=<%=UserID%>&UserOrgID=<%=UserOrgID%>&AsgChoice=<%=rs("OrganizationID")%>&ReportID=<%=ReportID%>&ReportName=<%=ReportName%>"><%=RS("OrganizationName")%>&nbsp</a><br>
	<%RS.MoveNext%>
<%Loop%>
