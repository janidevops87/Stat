...
'****************************************************************************************************
'Create Recordset
Set RS = Conn.Execute("sps_CONTRACTS_CriteriaGroups_TOC")%>
...
<a CLASS="SideBarData" TARGET=page href="CONTRACTS_CriteriaGroups_ByClient.sls?UserID=<%=UserID%>&UserOrgID=<%=UserOrgID%>&AsgChoice=0&ReportID=<%=ReportID%>&ReportName=<%=ReportName%>">All Clients</a><br>
<%Do While Not RS.EOF%>
	<a CLASS="SideBarData" TARGET=page href="CONTRACTS_CriteriaGroups_ByClient.sls?UserID=<%=UserID%>&UserOrgID=<%=UserOrgID%>&AsgChoice=<%=rs("CriteriaGroupID")%>&ReportID=<%=ReportID%>&ReportName=<%=ReportName%>"><%=RS("CriteriaGroupName")%>&nbsp</a><br>
	<%RS.MoveNext%>
<%Loop%>
