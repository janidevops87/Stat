...
<%'Statline page header%>
	<table width="100%">
		<tr><%'Insert the Name of the Report and the selected filter to the header%>
			...
			<td width="*" CLASS="PageTitle"><b><%=ReportName%><br><FONT CLASS=PageTitle2>&nbsp&nbsp<%=SetAsgChoice(AsgChoice)%></FONT>
		</tr>
	</table><p>
...
'****************************************************************************************************
'Create Recordset
vQuery = "sps_CONTRACTS_CriteriaGroups_ByClient [" & AsgChoice & "]"
'print vQuery
Set RS = Conn.Execute(vQuery)%>


<%Do While Not RS.EOF%>
	<%IF CriteriaID <> RS("CriteriaID") OR OrganizationID <> RS("OrganizationID") THEN%>
		<%HeaderRow%>
		<tr>
		<td width="200" CLASS="DataCell" valign="Top"><%=RS("CriteriaGroupName")%>&nbsp</td>
		<td width="190" CLASS="DataCell" valign="Top"><%=RS("OrganizationName")%>&nbsp</td>
		<td width="70"  CLASS="DataCell" valign="Top"><%=RS("DonorCategoryName")%>&nbsp</td>
		<td width="70"  CLASS="DataCell" valign="Top"><%=RS("CriteriaMaleAge")%>&nbsp</td>
		<td width="70"  CLASS="DataCell" valign="Top"><%=RS("CriteriaFemaleAge")%>&nbsp</td>
		<td width="62"  CLASS="DataCell" valign="Top"><%=RS("CriteriaWeight")%>&nbsp</td>
		</tr><tr>
		<%IF NOT ISNULL(rs("IndicationID")) THEN%>
			...
			<tr>
				<td width="200" CLASS="DataCell" valign="Top">&nbsp</td>
				<td width="190" CLASS="DataCell" valign="Top"><%=RS("IndicationName")%>&nbsp</td>
				<td width="272" CLASS="DataCell" valign="Top" colspan="4"><%=RS("IndicationNote")%>&nbsp</td>
			</tr>
		<%END IF%>
	<%ELSE%>
	    <tr>
...
Function HeaderRow%>
    ...
    <%IF NOT rs("ID")=1 THEN%><H5><%END IF%>
    <table border=1 width="662" cellSpacing=1 cellPadding=1 bordercolor="#F5EBDD" bordercolordark="#F5EBDD" bordercolorlight="#F5EBDD">
	<tr>
		<td width="200"  CLASS="DataHeading" valign="Top">Criteria Group Name</b></td>
		<td width="190"  CLASS="DataHeading" valign="Top">Schedule Group</u></b></td>
		<td width="60"   CLASS="DataHeading" valign="Top">Donor Category</u></b></td>
		<td width="50"   CLASS="DataHeading" valign="Top">Male Age</u></b></td>
		<td width="50"   CLASS="DataHeading" valign="Top">Female Age</u></b></td>
		<td width="50"   CLASS="DataHeading" valign="Top">Weight</u></b></td>
	</tr>
