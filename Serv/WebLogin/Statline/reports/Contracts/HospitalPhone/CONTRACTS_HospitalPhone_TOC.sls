...
'****************************************************************************************************
'Create Recordset
Set RS = Conn.Execute("CONTRACTS_HospitalPhone_TOC")%>

<table width="100%" CLASS="SideBarHeading">
	<tr>
		<td CLASS="SideBarHeading">Hospitals<br></td>
	</tr>
</table>

<a CLASS="SideBarData" TARGET=page href="CONTRACTS_HospitalPhone_ByOrg.sls?UserID=<%=UserID%>&UserOrgID=<%=UserOrgID%>&AsgChoice=*&ReportID=<%=ReportID%>&ReportName=<%=ReportName%>">All Hospitals</a><br>
<%Do While Not RS.EOF%>
	<a CLASS="SideBarData" TARGET=page href="CONTRACTS_HospitalPhone_ByOrg.sls?UserID=<%=UserID%>&UserOrgID=<%=UserOrgID%>&AsgChoice=<%=PatchURL(rs("OrganizationID"))%>&ReportID=<%=ReportID%>&ReportName=<%=ReportName%>"><%=RS("OrganizationName")%>&nbsp</a><br>
	<%RS.MoveNext%>
<%Loop%>
