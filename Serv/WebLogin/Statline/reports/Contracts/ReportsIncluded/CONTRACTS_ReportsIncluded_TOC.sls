...
'****************************************************************************************************
'Create Recordset
Set RS = Conn.Execute("CONTRACTS_ReportsIncluded_TOC")%>
...
<%Do While Not RS.EOF%>
	<a CLASS="SideBarData" TARGET=page href="CONTRACTS_ReportsIncluded_ByClient.sls?UserID=<%=UserID%>&UserOrgID=<%=UserOrgID%>&AsgChoice=<%=PatchURL(rs("OrganizationID"))%>&ReportID=<%=ReportID%>&ReportName=<%=ReportName%>"><%=RS("OrganizationName")%>&nbsp</a><br>
	<%RS.MoveNext%>
<%Loop%>
