...
Do While Not RS.EOF
	...
	<table width ="662" border="1" bordercolor="black">
		<tr>

			<%
			'Replace Carriage Returns with <BR>
			TempMsg1 = RS("AlertMessage1")
			TempMsg1 = Replace(TempMsg1, Chr(10), "<BR>", 1)

			TempMsg2 = RS("AlertMessage2")
			TempMsg2 = Replace(TempMsg2, Chr(10), "<BR>", 1)

			TempScMsg = RS("AlertScheduleMessage")
			TempScMsg = Replace(TempScMsg, Chr(10), "<BR>", 1)
			%>
			<td height="150" width="221"><font size="<%=FontSizeData%>" face="<%=FontNameData%>"><%=TempMsg1%></td>
			<td height="150" width="220"><font size="<%=FontSizeData%>" face="<%=FontNameData%>"><%=TempMsg2%></td>
			<td height="150" width="221"><font size="<%=FontSizeData%>" face="<%=FontNameData%>"><%=TempScMsg%></td>
		</tr>
	</table><%
	'Header Row
	Call HeaderRow()
