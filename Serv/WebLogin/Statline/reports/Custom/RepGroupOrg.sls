...
<%
pvReportGroup = FormatNumber(Request.QueryString("ReportGroupID"),0)
...
If GetChildren Then%>
	...
	<TABLE BORDER=0 WIDTH="100%" RULES="NONE" FRAME="VOID">
		...
		<TR>
			...
			<TD WIDTH="15%"><B><U><FONT SIZE="<%=FontSizeHead%>" FACE="<%=FontNameHead%>">Phone</TD>
		</TR>
		...
		<%For i = 0 to Ubound(vOrgList, 2)%>
			...
			<TR>
				...
				<TD VALIGN=TOP><FONT SIZE="<%=FontSizeData%>" FACE="<%=FontNameData%>"><%=vOrgList(8,i)%></TD>
			</TR>
...
Function GetChildren()
	...
	GetChildren = True
	 ...
	 vQuery = "sps_GetOrgList " & pvReportGroup
