...
<TABLE WIDTH = "100%" border ="0">
      <tr>
		...
		<TABLE>
			<TR><TD><FONT SIZE="2" FACE="<%=FontNameHead%>"><B><%=vMainTitle%></B></TD></TR>
			<TR><TD><FONT SIZE="2" FACE="<%=FontNameHead%>"><B><%=vReportTitle%></B></TD></TR>
		</TABLE>

        </td>
        <td width="20%" align="center">
         	<%=GetLogoUserOrg(PORGANIZATIONID)%>        	
        </td>
     <%if isdate(vTitlePeriod) and isdate(vTitleTo) Then %>   
        
        <td width="26%" valign="CENTER" ALIGN="RIGHT">
	    ...
	    <tr>
	      <td>
            ...
            <tr>
              <td colspan="2">
		<font size="2" face="<%=FontNameHead%>">
			<b>Ran At:&nbsp;</b>
		</font>
		<font size="2" face="<%=FontNameData%>">
			<%=FormatDateTime(Now, vbShortDate)& " " & FormatDateTime(Now, vbShortTime)%>
		</font>
		<BR>  
