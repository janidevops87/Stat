...
If AuthorizeMain Then
	...
	%>
		<!-- Print the header. -->
		<!--#include virtual="/loginstatline/reports/Custom/Head.sls"-->
					
		<!-- Get the counts of each alert group and referral type. -->
		<!--#include virtual="/loginstatline/reports/custom/Schedule_ActivitySection.sls"-->
	<%
	Else
	%>
		No Data Available
...
Function ExecuteMain()
	...
	' Query for Organization Time Zone if this should change
	' The sp query or the asp page will need updated to account for displaying times in a given time zone.
	vTZ = "MT" 
	
	'Get the schedule group name. Schedule groups are sub to procurement agencies
	...
	'Get data rows. 
	vQuery = "sps_GetSchedule '" & pvStartDate & "', '" & pvEndDate & "', " & pvScheduleGroupID
	'Response.Write vQuery
	Set RS = Conn.Execute(vQuery)
