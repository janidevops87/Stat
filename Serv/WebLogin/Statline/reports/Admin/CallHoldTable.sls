...
Dim vWeekDay
Dim i, j
Dim x, ConnDash, vReportGroupName
...
<TD VALIGN=TOP>
	...
	<TR>
		...
		<TD WIDTH="50%" ALIGN=LEFT>
			<TABLE> 
				<TR><TD ALIGN=LEFT><FONT SIZE="5" FACE="Arial Black"><B>Call Hold Distribution</TD></TR>
			</TABLE>
		</TD>
...
If vDetailQuery = 3 Then
	...
	IF vReportGroupID = 0 Then
		...
		vQuery =  "sps_ReportGroups " & pvUserOrgID
...
Select Case vDetailQuery
	...
	Case 4
		vQuery = "sps_HoldTimesTopAll '" & pvStartDate & "', '" & pvEndDate & "' "
End Select
...
%>
	...
	<%If vDetailQuery <> 4 Then%>
		<%IF vStartInterval = 0 and vEndInterval = 120 Then%>
			<TR><TD ALIGN=LEFT COLSPAN=4> <FONT FACE="Arial"><B> <%Response.Write(INT(vStartInterval) & " - > " & INT(vEndInterval) & " Seconds ")%></B></FONT> </TD></TR>
		<%ELSE%>
