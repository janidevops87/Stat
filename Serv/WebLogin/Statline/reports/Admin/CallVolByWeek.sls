...
<HTML>
<HEAD>
<TITLE>Call Volume By Week</TITLE>
</HEAD>
<BODY BGCOLOR="#F5EBDD">
<%	
Dim FontNameHead 
...
<TD VALIGN=TOP>
	...
	<TR>
		...
		<TD WIDTH="50%" ALIGN=LEFT>
			<TABLE> 
				<TR><TD ALIGN=LEFT><FONT SIZE="5" FACE="Arial Black"><B>Call Volumes By Week</TD></TR>
			</TABLE>
		</TD>
...
<%	
'Figure # of columns
vWeeks = DateDiff("ww", pvStartDate, pvEndDate)

'Set the array
ReDim CountArray(vRows + 1, vWeeks + 1)

'Initialize months
CountArray(0,0) = "Code/Week"

For j = 1 to vWeeks 
	CountArray(0,j) = Year(DateAdd("ww", j - 1, pvStartDate)) & "-wk " & DatePart("ww", DateAdd("ww", j - 1, pvStartDate))
Next

CountArray(0,vWeeks + 1) = "Total"
...
'Get data
vQuery = "sps_CallVolumeSC '" & pvStartDate & "', '" & pvEndDate & "', " & 1 & ", " & 2
...
For i = 1 to vRows
	For j = 1 to vWeeks
		CountArray(i,vWeeks + 1) = CountArray(i,vWeeks + 1) + CountArray(i,j)
	Next
Next

'Calculate Grand Total
For j = 1 to vWeeks
	CountArray(vRows+1, vWeeks+1) = CountArray(vRows+1, vWeeks+1) + CountArray(vRows+1, j)
Next
...
<TABLE WIDTH="100%" BORDER=0>
	...
	<TR><TD ALIGN=LEFT COLSPAN=4> <FONT FACE="Arial" SIZE="1">Sunday is first day of week. First and last weeks may not contain 7 days if date criteria do not begin and end on a Sunday.</FONT> </TD></TR>
</TABLE>
...
'Get data
vQuery = "sps_CallVolumeSC '" & pvStartDate & "', '" & pvEndDate & "', " & 2 & ", " & 2
Set RS = Conn.Execute(vQuery)
