			...
			<TR>
				<TD  valign=top>
					<%For ForLoopCounter = 0 to Ubound(ResultsArray,2)
						...
						<A ondblclick="processAccess(<%=ResultsArray(0,ForLoopCounter) & ", '" & escape(ResultsArray(1,ForLoopCounter)) &"', " & ResultsArray(2,ForLoopCounter) & ", " & pvScheduleGroupID %>)" readonly>
...
Function ExecuteMain()
	...
	'Get data rows
	vQuery = "sps_OnlineScheduleAccessList "  & pvReportGroupID & ", " & CInt(Request.QueryString("ScheduleAccess"))
...
<script language="JavaScript">
	function processAccess(PersonID, PersonName, AccessLevel, ScheduleGroupID)
	{
		...
		document.open(url,"ProcessScheduleAccess",features,0)
		return 0
