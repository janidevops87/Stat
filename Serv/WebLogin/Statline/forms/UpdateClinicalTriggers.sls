...
vReportName = "Update Clinical Triggers"
...
<form name="UpdateForm"
    action="<%=strHttpHeader & Request.ServerVariables("SERVER_NAME")%>/loginstatline/forms/ProcessClinicalTriggers.sls?ReportGroupID=<%=pvReportGroupID%>"
    method="POST"
...
<!-- End Header -->
	...
	<TR>
		...
		<TD ALIGN=Left width="11%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>Meet<BR>Clinical</B></TD>
		<TD ALIGN=Left width="11%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B># On Time</B></TD>
	...
	<TR><TD ALIGN=Left width="20%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>&nbsp;</B></TD>
		...
		<TD ALIGN=Left width="11%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>Triggers</B></TD>
		<TD ALIGN=Left width="11%"><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>Referrals</B></TD>
...
<%
	...
	'to be processed
	'vQuery = "sps_ReferralCompliance " & pvReportGroupID & ", '" & pvStartDate & "', '" & pvEndDate & "', " & pvOrgID
      	vQuery = "sps_GOLM_ClinicalTriggers " & pvReportGroupID & ", '" & pvStartDate & "', '" &  pvEndDate & "', " & pvOrgID
...
%>
		<TR>
			<TD <%=vCurrentColor%>><FONT SIZE="1" FACE="<%=FontNameHead%>">
				...
				If pvOrgID <> 0 Then
					...
					Response.Write (DateLiteralFull(RS("MonthID"),RS("YearID")))
				%>
			...
			<%vTotals(1) = vTotals(1) + RS("Referral_Count")%>
			<TD <%=vCurrentColor%>><FONT SIZE="1" FACE="<%=FontNameHead%>"><%=RS("Referral_Count")%></FONT></TD>
			
			<%vTotals(2) = vTotals(2) + RS("ClinicalTriggers_Count")%>
			<TD <%=vCurrentColor%>><FONT SIZE="1" FACE="<%=FontNameHead%>"><%=RS("ClinicalTriggers_Count")%></FONT></TD>
						
			<%If IsNull(RS("OnTimeReferrals_Count")) Then%>
				<TD <%=vCurrentColor%> ALIGN=Left><FONT SIZE="1" FACE="<%=FontNameHead%>">
				<input type="text" onfocus="this.select();" onblur="if(!isInteger(this.value)){alert('Please enter digits only');this.select();}" size="7" name="onTimeReferrals<%=i%>" maxlength="5" value="0"></FONT></TD>
			<%Else%>
				<TD <%=vCurrentColor%> ALIGN=Left><FONT SIZE="1" FACE="<%=FontNameHead%>">
				<input type="text" onfocus="this.select();" onblur="if (!isInteger(this.value)){alert('Please enter digits only');this.select();}" size="7" name="onTimeReferrals<%=i%>" maxlength="5" value="<%=RS("OnTimeReferrals_Count")%>"></FONT></TD>
			<%End If%>
			...
			Else
				vTotals(0) = vTotals(0) + RS("OnTimeReferrals_Count")
			End If
			%>
		</TR>
	...
	<TR><TD><FONT SIZE="1" FACE="<%=FontNameHead%>"><B>Totals</B></TD>
		...
		<TD ALIGN=Left><FONT SIZE="1" FACE="<%=FontNameHead%>"><B><%=vTotals(2)%></B></FONT></TD>		
