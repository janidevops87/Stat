/* CA DMVSUSPENSE_A */
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[DMVSUSPENSE_A]') and OBJECTPROPERTY(id, N'IsTable') = 1)
Begin
	PRINT 'Dropping DMVSUSPENSE_A Table'
	drop table [dbo].[DMVSUSPENSE_A]
End
GO

	PRINT 'Creating DMVSUSPENSE_A Table'
GO

CREATE TABLE [DMVSUSPENSE_A] (
	[ID] [int] NULL ,
