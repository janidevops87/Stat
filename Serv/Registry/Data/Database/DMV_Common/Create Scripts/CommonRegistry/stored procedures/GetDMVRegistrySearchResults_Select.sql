...
SELECT  DISTINCT
		...
		LEFT(DMV.FirstName, 20) AS 'RegistrySearchResultFirstName',
		LEFT(DMV.MiddleName, 20) AS 'RegistrySearchResultMiddleName',
		LEFT(DMV.LastName, 25) AS 'RegistrySearchResultLastName',
...
WHERE
	(@Donor IS NULL OR DMV.Donor=@Donor ) AND
	(@DOB IS NULL OR DMV.DOB = @DOB) AND
	(@License IS NULL OR PATINDEX(@License+ '%', DMV.License) > 0) AND
	...
	(@MiddleName IS NULL OR PATINDEX(@MiddleName+ '%',coalesce(DMV.MiddleName,'')) >0)  AND
