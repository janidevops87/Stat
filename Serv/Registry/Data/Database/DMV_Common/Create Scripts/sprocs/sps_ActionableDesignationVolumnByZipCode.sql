...
CREATE PROCEDURE sps_ActionableDesignationVolumeByZipCode
...
/* Limit Zip Codes to only 10 Higher and 10 Lower 
	...
	INSERT @ZipCodeTable
		SELECT TOP 10 
			ZipCode AS 'ZipCodes',
			SUM(DMVCount) + SUM(RegCount)AS 'ZipCodeCount',
			'Top 10 Highest Producing Zip Codes' AS 'HighestLowestName'
		 FROM @ZipCodeSelectTable
		 GROUP BY ZipCode
		 ORDER BY (SUM(DMVCount) + SUM(RegCount)) DESC
	...
	INSERT @ZipCodeTable
		SELECT TOP 10 
		 ...
		 FROM @ZipCodeSelectTable
		 GROUP BY ZipCode
		 ORDER BY (SUM(DMVCount) + SUM(RegCount))
