...
DECLARE @SelectionTable TABLE
	...
	INSERT @SelectionTable 
		...
		SELECT DISTINCT
			...
			Registry.SourceCode AS 'SourceCode',
		...
		FROM Registry
		LEFT JOIN RegistryADDR ON RegistryADDR.RegistryID = Registry.ID AND (RegistryADDR.AddrTypeID = 1)
		LEFT JOIN EventRegistry ON Registry.ID = EventRegistry.RegistryID
		LEFT JOIN EventCategory ON EventRegistry.EventCategoryID = EventCategory.EventCategoryID
	...
	INSERT @SelectionTable 
		...
		SELECT DISTINCT
			...
			/* Correlate Total Yes*/
			CASE 	WHEN (PreviousDonorStateDonorConfirmed Is Null AND (Donor = 1 AND DonorConfirmed = 1)) -- New Yes
						OR	(PreviousDonorStateDonor = 1 AND (Donor = 1 AND DonorConfirmed = 1)) -- Yes To Yes
						OR  (PreviousDonorStateDonor = 0 AND (Donor = 1 AND DonorConfirmed = 1)) -- No To Yes
						THEN 'Total Yes'
