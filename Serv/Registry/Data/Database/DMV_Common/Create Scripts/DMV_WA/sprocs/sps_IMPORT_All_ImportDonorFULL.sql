IF EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name = 'sps_IMPORT_All_ImportDonorFULL')
	BEGIN
		PRINT 'Dropping sps_IMPORT_All_ImportDonorFULL'
		DROP  Procedure  sps_IMPORT_All_ImportDonorFULL
	END
...
CREATE Procedure sps_IMPORT_All_ImportDonorFULL
AS
/******************************************************************************
...
while @@fetch_status = 0
  begin
    ...
    where IMPORT.ID = @ID;   
    if isnull(@Donor,'') <> ''
    insert DMVORGANTempTable (DMVID, ORGANTYPEID)
