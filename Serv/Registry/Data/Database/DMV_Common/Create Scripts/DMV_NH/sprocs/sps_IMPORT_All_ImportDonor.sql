...
while @@fetch_status = 0
  begin
    ...
    from IMPORT
    where IMPORT.ID = @ID;
    insert DMV ( IMPORTLOGID, SSN,  License, DOB,                 FirstName, MiddleName, LastName, Suffix, Gender, Donor,    RenewalDate,                 DeceasedDate,                       CSORState, CSORLicense, LastModified, CreateDate, PreviousDonorState)
    select      @IMPORTLOGID, NULL, LICENSE, CASE ISDATE(DOB)
                                             ...
                                             END,                 FIRST,     MIDDLE,     LAST,     SUFFIX, GENDER, @DonorTF, CASE ISDATE(RENEWALDATE)
                                                                                                                             ...
                                                                                                                             END,                         CASE ISDATE(DECEASEDDATE)
                                                                                                                                                          ...
                                                                                                                                                          END,                               CSORSTATE, CSORLICENSE, GetDate(), ISNULL(CREATEDATE,GETDATE()), PreviousDonorState
    from IMPORT
    where IMPORT.ID = @ID;
