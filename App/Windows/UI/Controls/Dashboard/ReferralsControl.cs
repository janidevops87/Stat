...
{
    public partial class ReferralsControl : BaseGridSearch
    {
        private SecurityHelper securityHelper;
        private GeneralConstant generalConstant = GeneralConstant.CreateInstance();
        private MainControl mainControl;

        public ReferralsControl()
        {
            InitializeComponent();
            InitializeBR(new ReferralBR());
            securityHelper = Security.SecurityHelper.GetInstance();
            if (mainControl == null)
                mainControl = new MainControl();
            //MainControl mainControl = mainControl as MainControl;
        ...
        protected override void ReloadGrid()
        {
            ...
            InitializeBR(new ReferralBR());
            ReferralBR referralBR = (ReferralBR)BusinessRule;
            DateTime testDT;
            referralBR.CallNumber = GetFilterConditionString("CallNumber");
            referralBR.StatAbbreviation = GetFilterConditionString("StateAbbrv");
            referralBR.OrganizationName = GetFilterConditionString("OrganizationName");
            referralBR.ReferralDonorFirstName = GetFilterConditionString("ReferralDonorFirstName");
            referralBR.ReferralDonorLastName = GetFilterConditionString("ReferralDonorLastName");
            referralBR.PreviousReferralTypeName = GetFilterConditionString("PrevReferralTypeName");
            referralBR.CurrentReferralTypeName = GetFilterConditionString("ReferralTypeName");
            referralBR.SourceCodeName = GetFilterConditionString("SourceCodeName");
            referralBR.StatEmployeeFirstName = GetFilterConditionString("StatEmployeeFirstName");
            referralBR.StatEmployeeLastName = GetFilterConditionString("StatEmployeeLastName");
            referralBR.StartDate = GetFilterConditionString("CallDateTime");

            if (referralBR.StartDate != null)
            {
                ultraGrid.DisplayLayout.Bands[0].Columns["CallDateTime"].Header.Caption = Convert.ToDateTime(referralBR.StartDate).ToString("MM/dd/yyyy HH:mm");
            }
            else
            {
                try
                {
                    ...
                    referralBR.StartDate = testDT.ToString();
                }
                catch
                {
            ...
            referralBR.EndDate = GetFilterConditionString("ToDate");
            if (referralBR.EndDate != null)
            {
                ultraGrid.DisplayLayout.Bands[0].Columns["ToDate"].Header.Caption = Convert.ToDateTime(referralBR.EndDate).ToString("MM/dd/yyyy HH:mm");
            }
            else
            {
                try
                {
                    ...
                    referralBR.EndDate = testDT.ToString();
                }
                catch
                {
            ...
            int lengthOfSearchFilter = GetLengthOfDateTimeFilter(referralBR);
            TimeSpan timediff = DateTime.Parse(referralBR.EndDate) - DateTime.Parse(referralBR.StartDate);
            if (timediff.Days > 90)
            {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (ultraGrid.Selected.Rows.Count == 1 && ultraGrid.Selected.Rows[0].Cells != null)
            {
                ...
                if (cellsCollection["ReferralDonorFirstName"].Value != DBNull.Value)
                    generalConstant.DeletePatientFirstName = (string)cellsCollection["ReferralDonorFirstName"].Value;
                if (cellsCollection["ReferralDonorLastName"].Value != DBNull.Value)
                    generalConstant.DeletePatientLastName = (string)cellsCollection["ReferralDonorLastName"].Value;
        ...
        private void ultraGrid_DoubleClickRow(object sender, DoubleClickRowEventArgs e)
        {
            ...
            {
                CellsCollection cellsCollection = ultraGrid.Selected.Rows[0].Cells;
                NavigateToScreen(AppScreenType.VBReferral,(int)cellsCollection[0].Value);
            }
            Cursor.Current = Cursors.Default;
        }
        private void ultraGrid_RightClick(object sender, EventArgs e)
        {
            ...
            {
                CellsCollection cellsCollection = ultraGrid.Selected.Rows[0].Cells;
                NavigateToScreen(AppScreenType.VBReferralReadOnly, (int)cellsCollection[0].Value);
            }
            Cursor.Current = Cursors.Default;
        }
        ...
        private int GetLengthOfDateTimeFilter(ReferralBR referralBR)
        {
            // start at one to allow 7 days minimum search
            int length = 1;
            if (referralBR.CallNumber != null)
            {
                // if 7 or more numbers are entered than ignore the date range.
                if (referralBR.CallNumber.Length > 6)
                {
                    return int.MaxValue;
                }
                else
                {
                    length += referralBR.CallNumber.Length;
                }
            }
            if (referralBR.OrganizationName != null)
                length += referralBR.OrganizationName.Length;
            if (referralBR.PreviousReferralTypeName != null)
                length += referralBR.PreviousReferralTypeName.Length;
            if (referralBR.ReferralDonorFirstName != null)
                length += referralBR.ReferralDonorFirstName.Length;
            if (referralBR.ReferralDonorLastName != null)
                length += referralBR.ReferralDonorLastName.Length;
            if (referralBR.SourceCodeName != null)
                length += referralBR.SourceCodeName.Length;
            if (referralBR.CurrentReferralTypeName != null)
                length += referralBR.CurrentReferralTypeName.Length;
            if (referralBR.StatAbbreviation != null)
                length += referralBR.StatAbbreviation.Length;
            if (referralBR.StatEmployeeFirstName != null)
                length += referralBR.StatEmployeeFirstName.Length;
            if (referralBR.StatEmployeeLastName != null)
                length += referralBR.StatEmployeeLastName.Length;
