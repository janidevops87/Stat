 IF EXISTS (SELECT * FROM dbo.sysobjects WHERE id = object_id (N'[dbo].[GetStatFileReferralEvent]') AND OBJECTPROPERTY(id, N'IsProcedure') = 1) 
BEGIN
	DROP PROCEDURE [dbo].[GetStatFileReferralEvent]
	PRINT 'Dropping Procedure: GetStatFileReferralEvent'
END
	PRINT 'Creating Procedure: GetStatFileReferralEvent'
GO

CREATE PROCEDURE [dbo].[GetStatFileReferralEvent]
(
...
/******************************************************************************
**		File: GetStatFileReferralEvent.sql
**		Name: GetStatFileReferralEvent
...
4. Call GetStatFileReferralEventSelect
...
-- Run the 	GetStatFileReferralEventSelect if @SeperateFiles = 0 and @DateTypeID = 1 (lastmodified)
if(@SeperateFiles = 0 AND @DateTypeID = 1 )
BEGIN
	EXEC GetStatFileReferralEventSelect @ExportFileID, @OrganizationID, 1
END
...
BEGIN
	...
	*/
		...
		WHERE fnEventList.LogEventID IN ( SELECT le.LogEventID FROM LogEvent le 
										JOIN Referral r ON r.CallID = le.CallID
	...
	*/
	...
	-- Step 4:
		EXEC GetStatFileReferralEventSelect @ExportFileID, @OrganizationID, @Created
...
END
-- Run the 	GetStatFileReferralEventSelect if @SeperateFiles = 0 and @DateTypeID = 0 (CreateDate)
if(@SeperateFiles = 0 AND @DateTypeID = 0 )
BEGIN
	EXEC GetStatFileReferralEventSelect @ExportFileID, @OrganizationID, 0
END
