

IF EXISTS (Select * FROM sys.objects WHERE type = 'P' AND name = 'PersonPhoneInsert')
	BEGIN
		PRINT 'Dropping Procedure PersonPhoneInsert'
		DROP Procedure PersonPhoneInsert
	END
GO

PRINT 'Creating Procedure PersonPhoneInsert'
GO
CREATE Procedure PersonPhoneInsert
(
		@PersonPhoneID int = null output,
		@PersonID int = null,
		@PhoneID int = null,
		@Phone varchar(15) = null,
		@PhoneTypeID int = null,
		@PhoneType varchar(100) = null,
		@Unused int = null,
		@PersonPhonePin varchar(10) = null,
		@PhonePin varchar(10) = null,
		@LastModified datetime = null,
		@PhoneAlphaPagerEmail varchar(100) = null,
		@UpdatedFlag smallint = null,
		@LastStatEmployeeID int = null,
		@AuditLogTypeID int = null,
		@PagerTypeID int = null,
		@PagerType varchar(100) = null,
		@EmailTypeID int = null,
		@EmailType varchar(100) = null,
		@AutoResponse bit = null					
)
AS
/******************************************************************************
**	File: PersonPhoneInsert.sql
**	Name: PersonPhoneInsert
**	Desc: Inserts PersonPhone Based on Id field 
**	Auth: Bret Knoll
**	Date: 10/6/2009
**	Called By: 
*******************************************************************************
**	Change History
*******************************************************************************
**	Date:			Author:					Description:
**	--------		--------				----------------------------------
**	10/6/2009		Bret Knoll			Initial Sproc Creation
**	07/12/2010		ccarroll			added this note for development build (GenerateSQL)
*******************************************************************************/
SET NOCOUNT ON
-- add upsert code here using Merge
DECLARE @PhoneTable TABLE
	(
	[PhoneID] [int] NOT NULL
	)

INSERT @PhoneTable (PhoneID)
EXEC [PhoneMerge] 
   @PhoneID = @PhoneID OUTPUT
  ,@Phone = @Phone
  ,@PhoneTypeID = @PhoneTypeID
  ,@LastModified = @LastModified
  ,@LastStatEmployeeID = @LastStatEmployeeID
  ,@AuditLogTypeID = @AuditLogTypeID
  ,@PhonePin = @PhonePin
  ,@ReturnNewRecord = 0
  
  
  


INSERT	PersonPhone
	(
		PersonID,
		PhoneID,
		Unused,
		PersonPhonePin,
		LastModified,
		PhoneAlphaPagerEmail,
		UpdatedFlag,
		LastStatEmployeeID,
		AuditLogTypeID,
		PagerTypeID,
		EmailTypeID,
		AutoResponse
	)
VALUES
	(
		@PersonID,
		@PhoneID,
		@Unused,
		@PersonPhonePin,
		@LastModified,
		@PhoneAlphaPagerEmail,
		@UpdatedFlag,
		@LastStatEmployeeID,
		ISNULL(@AuditLogTypeID, 1) /* insert */,
		@PagerTypeID,
		@EmailTypeID,
		@AutoResponse
	)

SET @PersonPhoneID = SCOPE_IDENTITY()

EXEC PersonPhoneSelect @PersonPhoneID

GO

GRANT EXEC ON PersonPhoneInsert TO PUBLIC
GO
