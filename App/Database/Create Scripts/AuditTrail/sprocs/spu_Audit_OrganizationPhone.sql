...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[spu_Audit_OrganizationPhone]') and OBJECTPROPERTY(id,
		N'IsProcedure') = 1)
	BEGIN
		PRINT 'Dropping Procedure spu_Audit_OrganizationPhone'
		drop procedure [dbo].[spu_Audit_OrganizationPhone]
	END
GO

	PRINT 'Creating Procedure spu_Audit_OrganizationPhone'
GO

create procedure [dbo].[spu_Audit_OrganizationPhone]  
(   
...
AND SUBSTRING(@Bitmap, 2, 1) & 2 <> 2 --SubLocationLevel   
)   
BEGIN   
...
+ ISNULL(CAST(@SubLocationLevel AS VARCHAR(1)), '')
...
CASE WHEN SUBSTRING(@Bitmap, 2, 1) & 2 = 2 AND ISNULL(@SubLocationLevel, '') = '' THEN 'ILB' ELSE @SubLocationLevel END  
