  IF  EXISTS (SELECT * FROM dbo.sysobjects WHERE id = OBJECT_ID(N'[dbo].[fn_rpt_MessageDateTimeConversion]') AND xtype in (N'FN', N'IF', N'TF'))
	BEGIN
		PRINT 'Dropping Function fn_rpt_MessageDateTimeConversion'
		DROP Function fn_rpt_MessageDateTimeConversion
	END
GO

PRINT 'Creating Function fn_rpt_MessageDateTimeConversion' 
GO
