...
public class NewReferralViewModel : IValidatableObject
{
    ...
    {
        var dateTimeWithTimeZoneMembers = GetDateTimeWithTimeZoneMembers();

        if (dateTimeWithTimeZoneMembers.Any())
        {
            yield return new ValidationResult(
                $"The date/time must not contain time zone information (including UTC).",
               dateTimeWithTimeZoneMembers);
        }
    }

    public IEnumerable<string> GetDateTimeWithTimeZoneMembers()
    {
        if (DonorAdmitDateTime is not null &&
            DonorAdmitDateTime.Value.Kind != DateTimeKind.Unspecified)
        {
            yield return nameof(DonorAdmitDateTime);
        }

        if (DonorDeathDateTime is not null &&
            DonorDeathDateTime.Value.Kind != DateTimeKind.Unspecified)
        {
            yield return nameof(DonorDeathDateTime);
        }

        if (DonorBrainDeathDateTime is not null &&
            DonorBrainDeathDateTime.Value.Kind != DateTimeKind.Unspecified)
        {
            yield return nameof(DonorBrainDeathDateTime);
        }

        if (DonorLsaDateTime is not null &&
            DonorLsaDateTime.Value.Kind != DateTimeKind.Unspecified)
        {
            yield return nameof(DonorLsaDateTime);
        }
    }
